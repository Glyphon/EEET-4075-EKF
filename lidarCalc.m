function [xHat, yHat, thetaHat] = lidarCalc(xHatPrior, yHatPrior, thetaHatPrior, lidarRanges, lidarRangeMax, lidarRangeMin, lidarAngleIncrement)
    ...
    % Estimate thetaHat
    thetaHatPrior = thetaHatPrior - (lidarBottExact - lidarBottRough)*lidarAngleIncrement;
    ...
    yHatTest = -(lidarBottDist^2 - lidarTopDist^2 - beaconBottom(2)^2 + beaconTop(2)^2)/(2*(beaconBottom(2) - beaconTop(2))) + sin(thetaHat)*lidarOffset;
    if isnan(yHatTest) || abs(yHatTest - yHatPrior) > 0.07
        ...
        if xHatPrior > 1.75
            xHat = beaconBottom(1) + sqrt(beta1*beta2)/(2*(beaconBottom(2) - beaconTop(2))) + cos(thetaHat)*lidarOffset;
        else
            xHat = beaconBottom(1) - sqrt(beta1*beta2)/(2*(beaconBottom(2) - beaconTop(2))) + cos(thetaHat)*lidarOffset;
        end
    end
